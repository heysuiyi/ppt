---
name: ppt-research
description: 将实际来源整理为 research/notes.md，区分已核实事实与待核实项
when_to_use: 用户要求调研、提供素材待整理，或任务的关键事实确实需要核实时
stages:
  - discover
  - author
---
...
由 Lead（主 Agent）维护精简的 `research/notes.md`——事实清单，不是报告。只有资料整理或关键事实核验确实需要时加载；不因制作 PPT 就自动扩大成研究项目。
...
3. 需要外部事实或最新资料时使用 `WebSearch`，优先直接且权威的来源；关键结论有冲突或证据不足时再交叉核验。需要视觉素材候选时可设置 `include_images: true`；图片结果仅用于发现，必须保留来源并核对授权后才能进入 deck。
4. 用 `WriteFile` 结构化写入 notes；每条事实标注实际来源 URL 或用户文件位置、适用日期和口径，便于页面文案回查。
5. 向用户摘要事实条数与待核实项。

## 约束

- 页数不决定是否调研；只补当前任务的事实缺口，所需证据足够就停止搜索。
- notes 是素材清单，不是幻灯片正文。
- 不把未核实数据写成定论。
