...
import { probeWorkspaceArtifactDetails } from "../presentation/workspace-artifacts";
import {
  capabilityGuidanceFromPlan,
  formatPptTaskPlanProjection,
  resolveSkillTiers,
} from "../task/ppt-task-composer";
import { deriveTaskFacts } from "../task/ppt-task-facts";
import { setPptTaskAssessmentSchema } from "../task/ppt-task-schema";
import { applyPptTaskAssessment } from "../task/ppt-task-session";
...
> = {
  ...
  mapResultToModelContent: (result) =>
    JSON.stringify({
      ...
      nextStep:
        result.plan.operation === "create" ||
        result.plan.operation === "edit" ||
        result.plan.operation === "restyle"
  ...
  execute: async (args, context) => {
    ...
    const facts = deriveTaskFacts({
      ...
      inputRevision: `request:${(context.request ?? "").length}:presentation:${requirePresentation(context).revision}`,
