...
export function composePptTaskPlan(input: {
  ...
  if (a.operation === "answer" || a.deliverable === "answer") {
    routes.push(
      route("answer", [], [], "solo", "ready", "普通问答：直接回答，不创建 capability。"),
    );
...
function route(
  ...
  return {
    id,
    now: unique(now),
    later: unique(later.filter((s) => !now.includes(s))),
    collab,
    availability,
    why,
    blockers,
...
function baseNow(op: Operation, a: TaskAssessment, f: TaskFacts): string[] {
...
function openBlockers(a: TaskAssessment): string[] {
  return (a.openQuestions ?? []).filter((q) => q.resolveBy === "user").map((q) => q.question);
}
...
function authoringRoutes(
  ...
  const mainWhy = wantsExport ? `${why}；先完成作者链并应用，再在 UI 导出 pptx` : why;
  ...
  if (a.collab === "required") {
    ...
    (a.collab === "allowed" || a.collab === "unspecified") &&
    f.canDelegate &&
    difficulty === "complex"
  ) {
    routes.push(
  ...
  if (a.wantsFix) {
    ...
    routes.push(
      ...base.map((r) => ({
        ...r,
        id:
          r.id === op || r.id === `${op}+export`
            ? `author-with-inspect${r.id.endsWith("+export") ? "+export" : ""}`
            : r.id,
...
function pickRoutes(
  ...
  if (a.collab === "required") {
    ...
    recommendedId =
      assist?.id ?? routes.find((r) => r.collab !== "solo")?.id ?? pool[0]?.id ?? null;
